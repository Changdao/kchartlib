...
public abstract class BaseKChartView extends ScrollAndScaleView {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        Log.i("ReactNativeJS KChartView","baseChartView onSizechanged:"+w+"  "+h);
    ...
    protected void onDraw(Canvas canvas) {
        //Log.i("KCHARTLIB","BaseKChartView onDraw don't call drawColor");
        super.onDraw(canvas);
        //Log.i("KCHARTLIB","Canvas clip bound:"+canvas.getClipBounds());
        //canvas.drawColor(mBackgroundPaint.getColor());
        if (mWidth == 0 || mMainRect.height() == 0 || mItemCount == 0) {
            //Log.i("KCHARTLIB","mWidth:"+mWidth);
            //if(mMainRect==null)Log.i("KCHARTLIB","mMainRect is null");
            //else Log.i("KCHARTLIB"," mMainRect.height():"+mMainRect.height()+" mItemCount:"+mItemCount);
            return;
        }
        calculateValue();
        canvas.save();
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 1; i < mGridColumns; i++) {
            ...
            if (translateX >= startX && translateX <= stopX) {
                ...
                String text = mAdapter.getDateString(index);
        ...
        if (translateX >= startX && translateX <= stopX) {
            ...
            canvas.drawText(getAdapter().getDateString(mStartIndex), 0, y, mTextPaint);
        }
        translateX = xToTranslateX(mWidth);
        if (translateX >= startX && translateX <= stopX) {
            //String text = formatDateTime(getAdapter().getDate(mStopIndex));
            String text = getAdapter().getDateString(mStopIndex);
